...
const Page = () => {
  ...
  const searchPaper = async () => {
    if (!debouncedSearch.trim()) return; // Prevent search if the query is empty
    setLoading(true);
    try {
      ...
      console.log(data);
      setPapers(data.results); // Updated to access `results` in the API response
    } catch (err) {
  ...
  // Debounce logic: Update debouncedSearch only after a delay
  useEffect(() => {
    ...
    return () => {
      clearTimeout(handler); // Cleanup the timeout on input change or component unmount
    };
  }, [search]);
    ...
    <div>
      ...
      <div className="relative">
        <Input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              setDebouncedSearch(search); // Trigger immediate search on Enter
            }
          }}
          className="border border-gray-300 rounded-lg p-2"
        />
        <button
          onClick={() => setDebouncedSearch(search)} // Trigger search immediately on button click
          disabled={loading}
      ...
      <div className="mt-6 space-y-2">
        {papers.length === 0 && debouncedSearch.length !== 0 && !loading ? (
            ...
            >
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      {paper.keywords.map((keyword, idx) => (
                        <div
                          key={idx}
                          className="bg-green-100 text-green-700 text-xs flex items-center gap-1 px-2 py-1 rounded-full"
                        >
                          <span>{keyword}</span>
                        </div>
                      ))}
              ...
              <div className="flex flex-col items-end justify-end">
                <Link
                  ...
                  className="text-base font-semibold bg-[#59C009] text-gray-50 px-5 py-2 flex items-center justify-center gap-1 rounded-full"
                >
